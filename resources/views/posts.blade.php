<x-layout>
  ...
  <div class="py-8 px-4 mx-auto max-w-7xl lg:py-6 lg:px-0">
      <div class="grid gap-10 md:grid-cols-2 lg:grid-cols-3">
    @foreach ($posts as $post)
      
    
    {{-- <article class="py-8 max-w-3xl border-b border-gray-500">
      <a href="/post/{{ $post['slug'] }}" class="hover:underline">
        ...
        <a href="/authors/{{ $post->author->username }}" class="hover:underline text-gray-500">
          {{ $post->author->name }} </a>
          In
          <a href="/categories/{{ $post->category->slug }}" class="text-gray-500 hover:underline">{{ $post->category->name }}</a> | {{ $post->created_at->diffForHumans() }}
      </div>
      ...
      <a href="/post/{{ $post['slug'] }}" class="font-medium text-blue-500 hover:underline">Read more &raquo;</a>
          ...
          <article class="p-6 bg-white rounded-lg border border-gray-200 shadow-md">
              ...
              <a href="/post/{{ $post['slug'] }}" class="hover:underline">
              <h2 class="mb-2 text-2xl font-bold tracking-tight text-gray-900">{{ $post->title }}
              </h2>
            </a>
              ...
              <div class="flex justify-between items-center">
                  ...
                  <a href="/post/{{ $post['slug'] }}" class="inline-flex items-center text-sm font-medium text-blue-600 hover:underline">
                      Read more
